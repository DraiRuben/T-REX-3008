...
#define AISLESPAWNERCOMPONENT_CREATE(GAMESCENE,PLAYER,ISFINALRUSH,GLOBALAGGRO)                                          \
	(SH3Component) {                                                            \
		.Terminate          = AisleSpawnerComponent_Terminate,                                 \
		.Update             = AisleSpawnerComponentUpdate,                  \
		.isInitialized      = false,             \
		.componentType      = AISLESPAWNERCOMPONENT_TYPEID,                                    \
		.properties         = AisleSpawnerComponent_CreateProperties(GAMESCENE,PLAYER,ISFINALRUSH,GLOBALAGGRO) \
	}
